    ...
    <div style={{ width: 260, background: 'var(--bg-sidebar)', display: 'flex', flexDirection: 'column', borderRight: '1px solid var(--border-light)', height: '100%' }}>
      ...
      <div style={{ padding: '0 12px', flex: 1, overflowY: 'auto' }}>
        ...
        <div style={{ marginTop: 12 }}>
          <button className="sidebar-item" onClick={() => setCurrentScreen('extensions-plugins')}>
          ...
          <div style={{ paddingLeft: 24, display: 'flex', flexDirection: 'column', gap: 2, borderLeft: '1px solid var(--border-strong)', marginLeft: 18, marginTop: 4, marginBottom: 4 }}>
            <button className={`sidebar-item ${currentScreen === 'extensions-plugins' ? 'active' : ''}`} onClick={() => setCurrentScreen('extensions-plugins')}>Plugins</button>
            <button className={`sidebar-item ${currentScreen === 'extensions-skills' ? 'active' : ''}`} onClick={() => setCurrentScreen('extensions-skills')}>Skills</button>
            <button className={`sidebar-item ${currentScreen === 'extensions-connectors' ? 'active' : ''}`} onClick={() => setCurrentScreen('extensions-connectors')}>Connectors</button>
          </div>
        </div>
        ...
        <div style={{ fontSize: 13, color: 'var(--text-secondary)', padding: '4px 4px 16px 4px', fontStyle: 'italic' }}>#</div>

        {/* Tasks List */}
        <div style={{ fontSize: 10, fontWeight: 600, color: '#555', textTransform: 'uppercase', marginBottom: 8, paddingLeft: 4 }}>Tasks</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <button className="sidebar-item" style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'block' }}>I WANT U T ORGANISE ALL MY SOCIAL M...</button>
          <button className="sidebar-item" style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'block' }}>organise entire my system files from mac d...</button>
          <button className="sidebar-item" style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'block' }}>Organise all files and folders and images ...</button>
          <button className="sidebar-item" style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'block' }}>是, 是</button>
          <button className="sidebar-item" style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'block' }}>Add a local remote control CLI Open Termi...</button>
          <button className="sidebar-item" style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'block' }}>contine with my request</button>
        </div>
      ...
      <div style={{ padding: '16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid var(--border-light)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div>
            <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-primary)' }}>Praveen k</div>
            <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>Pro trial Plan</div>
          </div>
        </div>
